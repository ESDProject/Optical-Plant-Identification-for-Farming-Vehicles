...
def load_label_files(file_name):
    names = {}
...
def draw_boxes(boxes, classes, scores, img, cls_names, detection_size, is_letter_box_image):
    ...
    font = ImageFont.truetype(r'Lato-Regular.ttf', 30)  
    ...
    for box, score, cls in zip(boxes, scores, classes):
        ...
        draw.text(box[:2], '{} {:.2f}%'.format(
            cls_names[cls], score * 100), fill=(255,255,0), font=font)
...
model_path = os.path.join(os.getcwd(), 'detect.tflite')
...
img = Image.open('test_images/SAM_1864.jpg')
...
boxes, classes, scores = handle_predictions(predictions[0],
                                            confidence=0.3,
                                            iou_threshold=0.5)
class_names = load_label_files("classes.txt")
draw_boxes(boxes, classes, scores, img, class_names, (height, width), True)
img.save("output.jpg")
print("Predicted image saved as output.jpg")